...
Generated in **Genspark** on **Nano Banana 2 Flash** (`gemini-3.1-flash-image`) — the model §2.1 of
the manifest targets, so the prompts and the `aspectRatio`/`imageSize` guidance apply as written.
Note that image generation there is rate-limited in five-hour windows, which is worth planning
re-runs around: a batch that runs out of credit mid-way silently falls back to a lower tier rather
than failing, which is how `page-disclaimer-hero` ended up at 2K.
...
The generator has confirmed one of these outright: `page-disclaimer-hero` is a 2K render, because the
4K redo hit a credit limit. For the rest it said only 'the latest 4K or 2K version' — which is not a
statement that they are 4K, so the dimensions still need checking one by one.
...
plenty. The images are being generated in Genspark, whose AI Drive can do this without a local
download; ask it to:

> Resize every image in /mycelia-bundle/species to 1024 px on the long edge, keep the filenames
> unchanged, write them to /mycelia-bundle/previews, and sync that folder to the shared Drive.

Locally it is one line either way:
